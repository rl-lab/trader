class Portfolio:
    ...
    def trade_to_position(self, position, price, trading_fees):
        ...
        if (position <= 0 and current_position < 0):
            ...
            interest_reduction_ratio = min(
                1, (position-1)/(current_position-1))
        if interest_reduction_ratio < 1:
            self.asset = self.asset - \
                (1-interest_reduction_ratio) * self.interest_asset
            self.fiat = self.fiat - \
                (1-interest_reduction_ratio) * self.interest_fiat
        ...
        # Proceed to trade
        asset_trade = (position * self.valorisation(price) /
                       price - self.asset)
        if asset_trade > 0:
            asset_trade = asset_trade / \
                (1 - trading_fees + trading_fees * position)
    ...
    def describe(self, price): print("Value : ", self.valorisation(
        price), "Position : ", self.position(price))
